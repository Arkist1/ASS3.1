...
try:
    ...
    def run_environment(
        episodes, max_steps, agent, last_steps_n, stop_score, do_train=True
    ):
        global max_returns
        ...
        for episode in range(episodes):
            ...
            print(
                colour + "Finished episode",
                " " * (3 - len(str(episode))),
                episode,
                "in",
                str(time_delta.seconds)
                + "."
                + str(time_delta.microseconds)[0:2]
                + "s\t",
    ...
    name = "06-20_23-39"  # name of the model
    ...
    if do_train and do_save:
        print("Saving model")
        main_agent.policy.save_model(
            f"models/model_{datetime.now().strftime('%m-%d_%H-%M')}.pt",
            f"models/model_target_{datetime.now().strftime('%m-%d_%H-%M')}.pt",
        )
        main_agent.save_memory(
            f"memory/memory_{datetime.now().strftime('%m-%d_%H-%M')}.jsonl"
        )

    print("saving report")
    save_report(
        main_agent, f"reports/report_{datetime.now().strftime('%m-%d_%H-%M')}.json"
    )
